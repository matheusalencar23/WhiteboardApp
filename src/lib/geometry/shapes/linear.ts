...
export function toWorldPoints(el: LinearShape): Point[] {
  return el.points.map((p) => ({ x: el.x + p.x, y: el.y + p.y }));
}

/** A forma como é armazenada, sem considerar rotação: bounding box dos pontos locais. */
export function getLinearGeometry(el: LinearShape): Bounds {
...
/** Onde a forma realmente aparece na tela, considerando rotação. */
export function getLinearBounds(el: LinearShape): Bounds {
...
export function linearContainsPoint(
  el: LinearShape,
  point: Point,
  threshold = 6,
): boolean {
